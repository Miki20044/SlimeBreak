...
public class BossPatternController : MonoBehaviour
{
    ...
    public float burstInterval = 0.17f;
    public float spamBulletSpeed = 4.5f;
    public float spamBulletScale = 0.2f;
    public float bulletOutwardTime = 1.7f;
    public float pauseBeforeReturn = 0.8f;
    public AudioClip returnBulletSound;

    [Header("Attack 4 - Laser")]
    public float laserWarningDuration = 1.1f;
    public float laserPreviewSpeed = 35f;
    public float laserStartRotateSpeed = 35f;
    public float laserRotateSpeed = 110f;
    public float laserEndRotateSpeed = 15f;
    public float laserAccelerationTime = 1.3f;
    public float laserDecelerationTime = 0.8f;
    public float laserRotationCount = 2f;
    public float laserDirectionPause = 0.8f;
    public AudioClip laserSound;

    [Header("Attack 5 - Falling Rain")]
    public int fallingTotalProjectiles = 52;
    public float fallingSpawnInterval = 0.085f;
    public float fallingBaseSpeed = 4f;
    public float fallingMaxSpeedMultiplier = 1.9f;
    public float fallingProjectileLifetime = 5f;
    public float fallingProjectileScale = 0.25f;
    public AudioClip fallingRainSound;

    [Header("Attack 6 - Bouncing Bullets")]
    public int bounceBulletCount = 15;
    public float bounceBulletSpeed = 5.75f;
    public int bounceMaxBounces = 3;
    public float bounceAttackDuration = 4.5f;
    public float bounceBulletScale = 0.2f;

    [Header("Attack 7 - Expanding Rings")]
    public int ringCount = 5;
    public int bulletsPerRing = 14;
    public float ringStagger = 0.6f;
    public float ringBulletSpeed = 4f;
    public float ringBulletLifetime = 3f;
    public float ringBulletScale = 0.2f;

    [Header("Attack 8 - Bomb Drop")]
    public int bombCount = 7;
    public float bombWarningTime = 1f;
    public float bombRadius = 1.5f;
    public int bombShrapnelCount = 10;
    public float bombShrapnelSpeed = 4.75f;
    public float bombShrapnelScale = 0.2f;

    [Header("Attack 9 - Walls + Dash Line")]
    public float wallThickness = 4f;
    public float wallGapWidth = 2.2f;
    public float wallCloseDuration = 1.7f;
    public float wallWarningDuration = 0.5f;
    public float dashLineThickness = 1.2f;
    public float dashLineWarningDuration = 0.4f;
    public float dashLineSweepDuration = 1.25f;

    [Header("Attack 10 - Dash Gauntlet")]
    public int gauntletLineCount = 6;
    public float gauntletLineThickness = 1.5f;
    public float gauntletSpawnInterval = 0.5f;
    public float gauntletLineWarning = 0.4f;
    public float gauntletLineDuration = 0.65f;

    [Header("Attack 11 - Death Curtain")]
    public int curtainBulletCount = 18;
    public int curtainGapSize = 2;
    public float curtainSpeed = 5.75f;
    public float curtainBulletScale = 0.35f;

    [Header("Attack 12 - Implosion")]
    public int implosionBulletCount = 20;
    public float implosionWarningTime = 1f;
    public float implosionSpeed = 4.75f;
    public float implosionBulletScale = 0.25f;

    [Header("Attack 13 - Forced Corridor")]
    public float corridorHeight = 2.2f;
    public float corridorWarning = 0.6f;
    public int corridorBulletCount = 22;
    public float corridorBulletInterval = 0.15f;
    public float corridorBulletSpeed = 7f;
    public float corridorBulletScale = 0.22f;
    ...
    IEnumerator RotateLasers(GameObject[] lasers, float dir, float totalDegrees)
    {
        ...
        float endSpeed = laserEndRotateSpeed * ProjMult();

        // dystans potrzebny na deceleracje (avg speed * time)
        float decelDistance = (maxSpeed + endSpeed) / 2f * laserDecelerationTime;

        float rotated = 0f;
        float elapsed = 0f;
        bool decelerating = false;
        float decelStartElapsed = 0f;
        ...
        while (rotated < totalDegrees)
        {
            ...
            float remaining = totalDegrees - rotated;
            if (!decelerating && remaining <= decelDistance)
            {
                decelerating = true;
                decelStartElapsed = elapsed;
            }

            if (decelerating)
            {
                float decT = Mathf.Clamp01((elapsed - decelStartElapsed) / laserDecelerationTime);
                currentSpeed = Mathf.Lerp(maxSpeed, endSpeed, decT);
            }
            else
            {
                float accelT = Mathf.Clamp01(elapsed / laserAccelerationTime);
                currentSpeed = Mathf.Lerp(startSpeed, maxSpeed, accelT);
            }
    ...
    IEnumerator Attack6_BouncingBullets()
    {
        for (int i = 0; i < bounceBulletCount; i++)
        {
            ...
            b.transform.localScale = Vector3.one * bounceBulletScale;
    ...
    IEnumerator Attack7_ExpandingRings()
    {
        for (int ring = 0; ring < ringCount; ring++)
        {
            ...
            for (int i = 0; i < bulletsPerRing; i++)
            {
                ...
                b.transform.localScale = Vector3.one * ringBulletScale;
    ...
    IEnumerator Attack8_BombDrop()
    {
        ...
        for (int i = 0; i < positions.Count; i++)
        {
            ...
            for (int j = 0; j < bombShrapnelCount; j++)
            {
                ...
                b.transform.localScale = Vector3.one * bombShrapnelScale;
                Destroy(b, 3f);
            }
        }
    ...
    IEnumerator Attack12_Implosion()
    {
        ...
        for (int i = 0; i < implosionBulletCount; i++)
        {
            ...
            b.transform.localScale = Vector3.one * implosionBulletScale;
            SetAlpha(b, 0.4f);
    ...
    IEnumerator Attack13_ForcedCorridor()
    {
        ...
        for (int i = 0; i < corridorBulletCount; i++)
        {
            ...
            b.transform.localScale = Vector3.one * corridorBulletScale;
            Destroy(b, 3f);
